...
const fetchData = async () => {
  ...
  //  let  url = config.baseURL+'/api/v1/safety';
   let  url = config.baseURL+'/api/v1/power-tools';
...
const deleteData = async (sID:string) => {
  try {
    ...
    let url = config.baseURL+"/api/v1/power-tools/delete/"+sID;
...
<template>
  <h2 class="mt-10 text-lg font-medium intro-y">Safety Of Power Tools List</h2>
  <div class="grid grid-cols-12 gap-6 mt-5">
    <div class="flex flex-wrap items-center col-span-12 mt-2 intro-y sm:flex-nowrap">
      <router-link :to="{ name: 'hse-safety-power-tools-create' }">
        <Button variant="primary" class="mr-2 shadow-md">
          Add New Safety Of Power Tools
        </Button>
      </router-link>
      <Menu>
      ...
      <Table class="border-spacing-y-[10px] border-separate -mt-2">
        <Table.Thead>
          <Table.Tr>
            ...
            <Table.Th class="text-left border-b-0 whitespace-nowrap uppercase">Tool ID Number</Table.Th>
            <Table.Th class="text-left border-b-0 whitespace-nowrap uppercase">Tool Name</Table.Th>
            <Table.Th class="text-left border-b-0 whitespace-nowrap uppercase">Tool Type</Table.Th>
            <Table.Th class="text-left border-b-0 whitespace-nowrap uppercase">Tool Manufacturer</Table.Th>
            <Table.Th class="text-left border-b-0 whitespace-nowrap uppercase">Tool User</Table.Th>
            <Table.Th class="text-left border-b-0 whitespace-nowrap uppercase">Tool Last Calibration Date</Table.Th>
            <Table.Th class="text-left border-b-0 whitespace-nowrap uppercase">Tool Last Maintenance Date</Table.Th>
            <Table.Th class="text-left border-b-0 whitespace-nowrap uppercase">Tool Enlistment Date</Table.Th>
        ...
        <Table.Tbody v-if="state.visitorsEntry.length !== 0">
          <Table.Tr  v-for="(report, index) in state.visitorsEntry" :key="index" class="intro-x">
            ...
            <Table.Td class="whitespace-nowrap box w-40 text-left rounded-l-none rounded-r-none border-x-0 shadow-[5px_3px_5px_#00000005] first:rounded-l-[0.6rem] first:border-l last:rounded-r-[0.6rem] last:border-r dark:bg-darkmode-600">
              {{ report.tool_last_calibration_date }}
            </Table.Td>
            <Table.Td class="whitespace-nowrap box w-40 text-left rounded-l-none rounded-r-none border-x-0 shadow-[5px_3px_5px_#00000005] first:rounded-l-[0.6rem] first:border-l last:rounded-r-[0.6rem] last:border-r dark:bg-darkmode-600">
              {{ report.tool_last_maintenance_date }}
            </Table.Td>
            <Table.Td class="whitespace-nowrap box w-40 text-left rounded-l-none rounded-r-none border-x-0 shadow-[5px_3px_5px_#00000005] first:rounded-l-[0.6rem] first:border-l last:rounded-r-[0.6rem] last:border-r dark:bg-darkmode-600">
              {{ report.tool_enlistment_date }}
            </Table.Td>
            <Table.Td
              :class="[
            ...
            >
              <div class="flex items-center justify-center">
                ...
                <router-link :to="{name:'hse-safety-power-tools-edit', params:{id:report.id} }" class="flex items-center mr-3">
                  <Lucide icon="CheckSquare" class="w-4 h-4 mr-1" />
                  Edit
                </router-link>
                <a
