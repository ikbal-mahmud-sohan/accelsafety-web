...
const submitForm = async () => {
  ...
  cvformData.created_by = "1";
    ...
    if (validate.value.$invalid) {
            ...
            try {
                let  url = config.baseURL+'/api/v1/hse-entry-confined-space';
...
const updateForm = async () => {
  ...
  cvUpdateformData.updated_by = "2";
    ...
    if (validate2.value.$invalid) {
            ...
            try {
                let  url = config.baseURL+'/api/v1/hse-entry-confined-space/'+cvformData.id;
...
const ApprovedCv = async () => {
  ...
  try {
                let  url = config.baseURL+'/api/v1/hse-entry-confined-space-status/'+cvformData.id;
                const response = await axios.post(url, cvUpdateformData, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                },
  ...
  <div class="grid grid-cols-12 gap-6 mt-8 relative">
    ...
    <div class="absolute top-0 left-0 bg-white z-50 shadow-md rounded-md p-4 w-full">
      ...
      <div  class="py-2 px-2 flex justify-between items-center">
          <div class="flex w-full justify-between items-center">
            ...
            <div v-for="( data , index) in state.viewData">
              ...
              Update Entry into Confined Space
            </Button>
            </div>
      ...
      <div class="p-4" v-if="state.AddControlVisitorsHSE01">
        ...
        <div class="py-4"><h2 class="mr-auto text-xl text-center font-medium uppercase">Insert Entry into Confined Space</h2></div>
          <Alert variant="primary" dismissible class="col-span-11 mb-6 intro-y box dark:border-darkmode-600" v-slot="{ dismiss }">
              <div class="flex items-center">
                ...
                <span>
                  Ensure accurate online Entry into Confined Space with correct date formats and file uploads in the specified format and size.
                </span>
      ...
      <div class="p-4" v-if="state.UpdateControlVisitorsHSE01">
        <div class="p-5 mt-5 intro-y box">
        <div class="py-4"><h2 class="mr-auto text-xl text-center font-medium uppercase">Update Entry into Confined Space</h2></div>
          <Alert variant="primary" dismissible class="col-span-11 mb-6 intro-y box dark:border-darkmode-600" v-slot="{ dismiss }">
              <div class="flex items-center">
                <span>
                  <Lucide icon="Info" class="w-4 h-4 mr-2" />
