...
const fetchData = async () => {
  try {
   let  url = config.baseURL+'/api/v1/emergency-exit-light';
...
const deleteData = async (sID:string) => {
  try {
    let url = config.baseURL+"/api/v1/emergency-exit-light/delete/"+sID;
...
<template>
  <h2 class="mt-10 text-lg font-medium intro-y">Emergency Exit Light List</h2>
          ...
          <Table.Tr v-for="(report, index) in state.emergencyAlarm" :key="index" class="intro-x">
            ...
            >
              <div class="flex items-center justify-center">
                ...
                <router-link :to="{name:'hse-emergency-exit-light-full', params:{id:report.id} }" class="flex items-center mr-3 text-nowrap">
                  <Lucide icon="CheckSquare" class="w-4 h-4 mr-1 " />
                  Update
                </router-link>
                <router-link :to="{name:'hse-emergency-exit-light-edit', params:{id:report.id} }" class="flex items-center mr-3 text-nowrap">
