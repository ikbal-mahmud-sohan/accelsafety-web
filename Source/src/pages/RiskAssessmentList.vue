...
const fetchData = async () => {
  try {
   let  url = config.baseURL+'/api/v1/hira-lites-assessment';
    ...
    state.riskAssessment = response.data.data;
  } catch (error) {
...
<template>
  <h2 class="mt-10 text-lg font-medium intro-y">Risk Assessment List</h2>
      ...
      <Table class="border-spacing-y-[10px] border-separate -mt-2">
        <Table.Thead>
          <Table.Tr>
            ...
            <Table.Th class="text-left border-b-0 whitespace-nowrap uppercase"> Site Location </Table.Th>
            <Table.Th class="text-left border-b-0 whitespace-nowrap uppercase"> Activity or Task </Table.Th>
            <Table.Th class="text-left border-b-0 whitespace-nowrap uppercase"> Risk Assessment Conducted by </Table.Th>
            <Table.Th class="text-left border-b-0 whitespace-nowrap uppercase"> Process Owner / Department </Table.Th>
            <Table.Th class="text-left border-b-0 whitespace-nowrap uppercase"> Date Conducted </Table.Th>
            <Table.Th class="text-left border-b-0 whitespace-nowrap uppercase"> Next Review Date </Table.Th>
            <Table.Th class="text-center border-b-0 whitespace-nowrap uppercase">ACTIONS</Table.Th>
          </Table.Tr>
        </Table.Thead>
        <Table.Tbody>
          <Table.Tr v-for="(report, index) in state.riskAssessment" :key="index" class="intro-x">
            ...
            <Table.Td class="whitespace-nowrap box w-40 text-left rounded-l-none rounded-r-none border-x-0 shadow-[5px_3px_5px_#00000005] first:rounded-l-[0.6rem] first:border-l last:rounded-r-[0.6rem] last:border-r dark:bg-darkmode-600">
              {{ report.activity_or_task }}
            </Table.Td>
           
            
            <Table.Td class="whitespace-nowrap box w-40 text-left rounded-l-none rounded-r-none border-x-0 shadow-[5px_3px_5px_#00000005] first:rounded-l-[0.6rem] first:border-l last:rounded-r-[0.6rem] last:border-r dark:bg-darkmode-600">
              {{ report.risk_assessment_conducted_by }}
            </Table.Td>
            <Table.Td class="whitespace-nowrap box w-40 text-left rounded-l-none rounded-r-none border-x-0 shadow-[5px_3px_5px_#00000005] first:rounded-l-[0.6rem] first:border-l last:rounded-r-[0.6rem] last:border-r dark:bg-darkmode-600">
              {{ report.process_owner_or_department }}
            </Table.Td>
