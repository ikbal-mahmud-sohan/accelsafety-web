...
const fetchData = async () => {
  try {
   let  url = config.baseURL+'/api/v1/emergency-alarm-audible';
    const response = await axios.get(url,{
                headers: {
                    'Authorization': state.token,
                },
                });
    state.emergencyAlarm = response.data.data;
  } catch (error) {
...
const deleteData = async (sID:string) => {
  try {
    let url = config.baseURL+"/api/v1/emergency-alarm-audible/delete/"+sID;
...
<template>
  <h2 class="mt-10 text-lg font-medium intro-y">Emergency Alarm Audible List</h2>
      ...
      <Table class="border-spacing-y-[10px] border-separate -mt-2">
        <Table.Thead>
          <Table.Tr>
            ...
            <Table.Th class="text-left border-b-0 whitespace-nowrap uppercase"> Tag No. </Table.Th>
            <Table.Th class="text-left border-b-0 whitespace-nowrap uppercase"> Type </Table.Th>
        ...
        <Table.Tbody>
          <Table.Tr v-for="(report, index) in state.emergencyAlarm" :key="index" class="intro-x">
            ...
            >
              <div class="flex items-center justify-center">
                ...
                <router-link :to="{name:'hse-emergency-alarm-audible-full', params:{id:report.id} }" class="flex items-center mr-3 text-nowrap">
                  <Lucide icon="CheckSquare" class="w-4 h-4 mr-1 " />
                  Update
                </router-link>
                <router-link :to="{name:'hse-emergency-alarm-audible-edit', params:{id:report.id} }" class="flex items-center mr-3 text-nowrap">
