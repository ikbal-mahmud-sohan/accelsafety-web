...
const submitForm = async () => {
  ...
  if (validate.value.$invalid) {
    ...
    try {
      let url = config.baseURL + '/api/v1/hse-safety-committee-meeting-minutes-doc';
...
const updateForm = async () => {
  ...
  if (validate2.value.$invalid) {
    ...
    try {
      let url = config.baseURL + '/api/v1/hse-safety-committee-meeting-minutes-doc/update/' + cvformData.id;
...
const fetchcvUpdateData = async () => {
  try {
    ...
    let url = config.baseURL + '/api/v1/hse-safety-committee-meeting-minutes-doc/show/' + cvformData.id;
    const response = await axios.get(url, {
      headers: {
        'Authorization': state.token,
      },
    ...
    <div class=" bg-white z-50 shadow-md rounded-md p-4 w-full">
      ...
      <div class="p-4" v-if="state.AddControlVisitorsHSE01">
        <div class="p-5 mt-5 intro-y box">
          <div class="py-4">
            <h2 class="mr-auto text-xl text-center font-medium uppercase">Insert Safety Committee Meeting Minutes Doc</h2>
          </div>
          <Alert variant="primary" dismissible class="col-span-11 mb-6 intro-y box dark:border-darkmode-600"
            ...
            <div class="flex items-center">
              ...
              <span>
                Ensure accurate online Safety Committee Meeting Minutes with correct date formats and file uploads in the specified
                format and size.
              </span>
        ...
        <div class="flex flex-col justify-end gap-2 mt-5 md:flex-row">
          <Button @click="CloseControlVisitorsHSE01" type="button"
      ...
      <div class="p-4" v-if="state.UpdateControlVisitorsHSE01">
        <div class="p-5 mt-5 intro-y box">
          <div class="py-4">
            <h2 class="mr-auto text-xl text-center font-medium uppercase">Update Safety Committee Meeting Minutes doc</h2>
          </div>
          <Alert variant="primary" dismissible class="col-span-11 mb-6 intro-y box dark:border-darkmode-600"
            v-slot="{ dismiss }">
            <div class="flex items-center">
          ...
          <div class="p-5 border rounded-md border-slate-200/60 dark:border-darkmode-400">
            ...
            <div class="mt-5">
              ...
              <FormInline class="flex flex-wrap items-center pt-5 mt-5 xl:flex-row first:mt-0 first:pt-0">
                ...
                <div class="flex-1 w-full mt-3 xl:mt-0">
                  <ClassicEditor v-model="updateeditorData" :class="{ 'border-danger': validate2.descriptions.$error, }"
                    :config="editorConfig" />

                  <div class="flex justify-between">
                    <template v-if="validate2.descriptions.$error">
                      <div v-for="(error, index) in validate2.descriptions.$errors" :key="index"
                        class="mt-2 text-danger whitespace-nowrap">
                        {{ error.$message }}
                      </div>
                    </template>
