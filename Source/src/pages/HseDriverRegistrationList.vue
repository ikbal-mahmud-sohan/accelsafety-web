...
const fetchData = async () => {
  ...
  //  let  url = config.baseURL+'/api/v1/safety';
   let  url = config.baseURL+'/api/v1/driver';
...
const deleteData = async (sID:string) => {
  try {
    ...
    let url = config.baseURL+"/api/v1/driver/delete/"+sID;
...
<template>
  <h2 class="mt-10 text-lg font-medium intro-y">Driver Registration List</h2>
          ...
          <Table.Tr  v-for="(report, index) in state.visitorsEntry" :key="index" class="intro-x">
            ...
            >
              <div class="flex items-center justify-center">
                ...
                <router-link :to="{name:'hse-driver-registration-edit', params:{id:report.id} }" class="flex items-center mr-3">
                  <Lucide icon="CheckSquare" class="w-4 h-4 mr-1" />
                  Edit
                </router-link>
                <a
