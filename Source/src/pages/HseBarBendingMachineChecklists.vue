...
const deleteData = async (sID:string) => {
  try {
    let url = config.baseURL+"/api/v1/bar-bending-machine-checklists/"+sID;
...
<template>
  <h2 class="mt-10 text-lg font-medium intro-y">List Bar Bending Machine</h2>
  <div class="grid grid-cols-12 gap-6 mt-5">
    <div class="flex flex-wrap items-center col-span-12 mt-2 intro-y sm:flex-nowrap">
      <router-link :to="{ name: 'hse-bar-bending-machine-checklists-create' }">
        <Button variant="primary" class="mr-2 shadow-md">
          Add Bar Bending Machine
        </Button>
      </router-link>
      <Menu>
