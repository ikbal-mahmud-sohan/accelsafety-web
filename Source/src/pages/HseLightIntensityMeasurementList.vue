...
const deleteData = async (sID:string) => {
  try {
    let url = config.baseURL+"/api/v1/hse-light-intensity-measurement/"+sID;
...
<template>
  <h2 class="mt-10 text-lg font-medium intro-y">Light Intensity Measurement List</h2>
  <div class="grid grid-cols-12 gap-6 mt-5">
    <div class="flex flex-wrap items-center col-span-12 mt-2 intro-y sm:flex-nowrap">
      <router-link :to="{ name: 'hse-light-intensity-measurement-create' }">
        <Button variant="primary" class="mr-2 shadow-md">
          Add Light Intensity Measurement
        </Button>
      </router-link>
      <Menu>
      ...
      <Table class="border-spacing-y-[10px] border-separate -mt-2">
        <Table.Thead>
          <Table.Tr>
            ...
            <Table.Th class="text-left border-b-0 whitespace-nowrap uppercase">Area</Table.Th>
            <Table.Th class="text-left border-b-0 whitespace-nowrap uppercase">Location</Table.Th>
            <Table.Th class="text-left border-b-0 whitespace-nowrap uppercase">Location ID</Table.Th>
            <Table.Th class="text-left border-b-0 whitespace-nowrap uppercase">Day Time Reading (Lux)</Table.Th>
            <Table.Th class="text-left border-b-0 whitespace-nowrap uppercase">Night Time Reading (Lux)</Table.Th>
            <Table.Th class="text-left border-b-0 whitespace-nowrap uppercase">Date of Test</Table.Th>
            <Table.Th class="text-left border-b-0 whitespace-nowrap uppercase">Day Time (350 Lux)</Table.Th>
            <Table.Th class="text-left border-b-0 whitespace-nowrap uppercase">Night Time (350 Lux)</Table.Th>
        ...
        <Table.Tbody v-if="state.noiseIntensityMeasurementData.length !== 0">
          <Table.Tr  v-for="(report, index) in state.noiseIntensityMeasurementData" :key="index" class="intro-x">
            ...
            <Table.Td class="whitespace-nowrap box w-40 text-left rounded-l-none rounded-r-none border-x-0 shadow-[5px_3px_5px_#00000005] first:rounded-l-[0.6rem] first:border-l last:rounded-r-[0.6rem] last:border-r dark:bg-darkmode-600">
              {{ report.day_time_reading_lux }}
            </Table.Td>
            <Table.Td class="whitespace-nowrap box w-40 text-left rounded-l-none rounded-r-none border-x-0 shadow-[5px_3px_5px_#00000005] first:rounded-l-[0.6rem] first:border-l last:rounded-r-[0.6rem] last:border-r dark:bg-darkmode-600">
              {{ report.night_time_reading_lux }}
            </Table.Td>
            <Table.Td class="whitespace-nowrap box w-40 text-left rounded-l-none rounded-r-none border-x-0 shadow-[5px_3px_5px_#00000005] first:rounded-l-[0.6rem] first:border-l last:rounded-r-[0.6rem] last:border-r dark:bg-darkmode-600">
              {{ report.date_of_test }}
            </Table.Td>
            <Table.Td class="box w-40 text-left rounded-l-none rounded-r-none border-x-0 shadow-[5px_3px_5px_#00000005] first:rounded-l-[0.6rem] first:border-l last:rounded-r-[0.6rem] last:border-r dark:bg-darkmode-600">
              {{ report.day_time_350_lux }}
            </Table.Td>
            <Table.Td class="box w-40 text-left rounded-l-none rounded-r-none border-x-0 shadow-[5px_3px_5px_#00000005] first:rounded-l-[0.6rem] first:border-l last:rounded-r-[0.6rem] last:border-r dark:bg-darkmode-600">
              {{ report.night_time_350_lux }}
            </Table.Td>
