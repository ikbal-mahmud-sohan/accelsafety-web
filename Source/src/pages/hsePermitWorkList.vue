...
const deleteData = async (sID:string) => {
  try {
    let url = config.baseURL+"/api/v1/hse-permit-work-form/"+sID;
...
<template>
  <h2 class="mt-10 text-lg font-medium intro-y">List Permit To Work</h2>
      ...
      <Table class="border-spacing-y-[10px] border-separate -mt-2">
        <Table.Thead>
          <Table.Tr>
            ...
            <Table.Th class="text-left border-0 whitespace-nowrap uppercase">Permit No</Table.Th>
            <Table.Th class="text-left border-0 whitespace-nowrap uppercase">Issue Date</Table.Th>
            <Table.Th class="text-left border-0 whitespace-nowrap uppercase">From Dept. Name</Table.Th>
            <Table.Th class="text-left border-0 whitespace-nowrap uppercase">To Dept. Name</Table.Th>
            <Table.Th class="text-left border-0 whitespace-nowrap uppercase">Issuer</Table.Th>
        ...
        <Table.Tbody v-if="state.noiseIntensityMeasurementData.length !== 0">
          <Table.Tr  v-for="(report, index) in state.noiseIntensityMeasurementData" :key="index" class="intro-x">
            ...
            <Table.Td class="whitespace-nowrap box text-left rounded-l-none rounded-r-none border-x-0 shadow-[5px_3px_5px_#00000005] first:rounded-l-[0.6rem] first:border-l last:rounded-r-[0.6rem] last:border-r dark:bg-darkmode-600">
              {{ report.ptw_from_dept_name }}
            </Table.Td>
            <Table.Td class="whitespace-nowrap box w-40 text-left rounded-l-none rounded-r-none border-x-0 shadow-[5px_3px_5px_#00000005] first:rounded-l-[0.6rem] first:border-l last:rounded-r-[0.6rem] last:border-r dark:bg-darkmode-600">
              {{ report.ptw_to_dept_name }}
            </Table.Td>
            <Table.Td class="whitespace-nowrap box w-40 text-left rounded-l-none rounded-r-none border-x-0 shadow-[5px_3px_5px_#00000005] first:rounded-l-[0.6rem] first:border-l last:rounded-r-[0.6rem] last:border-r dark:bg-darkmode-600">
              {{ report.ptw_issuer }}
            </Table.Td>
