...
const fetchData = async () => {
  try {
   let  url = config.baseURL+'/api/v1/fire-extinguisher-ctg';
...
const deleteData = async (sID:string) => {
  try {
    let url = config.baseURL+"/api/v1/fire-extinguisher-ctg/delete/"+sID;
...
<template>
  <h2 class="mt-10 text-lg font-medium intro-y">Fire Extinguisher CTG List</h2>
          ...
          <Table.Tr v-for="(report, index) in state.riskAssessment" :key="index" class="intro-x">
            ...
            >
              <div class="flex items-center justify-center">
                ...
                <router-link :to="{name:'hse-fire-extinguisher-ctg-full', params:{id:report.id} }" class="flex items-center mr-3 text-nowrap">
                  <Lucide icon="CheckSquare" class="w-4 h-4 mr-1 " />
                  Update
                </router-link>
                <router-link :to="{name:'hse-fire-extinguisher-ctg-edit', params:{id:report.id} }" class="flex items-center mr-3 text-nowrap">
