...
const submitForm = async () => {
  ...
  formData.emp_id = [...selectedEmployeeIds.value]; // Ensure this is an array
  
  const form = new FormData();

  // Append all keys except 'signature'
  (Object.keys(formData) as Array<keyof typeof formData>).forEach((key) => {
    if (key === 'emp_id' && Array.isArray(formData.emp_id)) {
      // Append each array element as 'emp_id[]'
      formData.emp_id.forEach((id) => form.append('emp_id[]', id));
  ...
  console.log('Form Data for Submission:', [...form.entries()]); // Debugging the final form data
  ...
  if (validate.value.$invalid) {
    ...
    try {
      const url = config.baseURL + '/api/v1/training-attendence-multiple';
...
<template>
  <div class="flex items-center mt-8 intro-y">
    <h2 class="mr-auto text-lg font-medium">Multiple Training Attendence</h2>
  </div>
  <div class="grid grid-cols-11 pb-20 mt-5 gap-x-6">
    ...
    >
      <div class="flex items-center">
        ...
        <span>
          Multiple Training Attendence topics to employees by matching their IDs with the relevant training sessions.
          <a
    ...
    <div class="hidden col-span-2 intro-y 2xl:block">
      <div class="sticky top-0 pt-10">
        ...
        >
          ...
          >
            <a href="">Multiple Training Attendence</a>
          </li>
          <li
        ...
        >
          ...
          >
            <div>
              When filling out the Multiple Training Attendence report, be specific and clear with details, using the correct date format and precise descriptions..
            </div>
            <div class="mt-2">
